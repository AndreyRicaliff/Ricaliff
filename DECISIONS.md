...
## 2026-07-15 — [infra] Registro de sessão no seed via script, disparado pela /revisao
**Problema:** o histórico de sessões do hub congelou em maio e só descongelou por pedido manual — o hub dizia "12 sessões" enquanto o git tinha ~950 commits em 5 projetos. Pior: `ag-converge` tinha 71 commits e 1 sessão; `pulsar-finance` (225 com o painel anterior) e `algoritmo-lideranca` (9) não existiam nem como projeto, então nenhuma sessão podia ancorar neles. · **Opções:** A) continuar editando `seed.js` na mão quando lembrar · B) hook de git que registra sessão por commit · C) script validador + passo de fecho obrigatório na `/revisao`. · **Decisão:** C. · **Por quê:** (A) é o que falhou — depende de eu lembrar. (B) confunde as duas granularidades: commit ≠ sessão; o XP já é por commit (`ag-hub-sync.ps1`) e sessão é bloco defensável em entrevista — automatizar por commit produziria 950 sessões de ruído. (C) engancha na `/revisao`, que JÁ varre o git do projeto: o custo marginal é ~zero e o agrupamento em blocos continua sendo julgamento, não regex. · **Consequências:** sessão só entra se uma revisão rodar — projeto sem revisão continua defasando (aceito: sem revisão não há o que defender mesmo). O script é o portão: barra vazamento de cliente, `projectId` órfão e seed com sintaxe inválida (reverte). Bump de `agh_seed_v` é o que propaga pra browser já seedado. · **Em entrevista (30s):** "O histórico do meu hub apodrecia porque atualizar era manual. Em vez de automatizar por commit — que confundiria granularidade e viraria ruído — enganchei num ritual que já existia e já lia o git: a revisão. E botei a validação num script em vez da minha memória: o repo é público, então vazamento de nome de cliente precisa de portão automático, não de disciplina."

## 2026-07-22 — [fx] Camada de encantamento nível site-imersivo (fx.js aditivo)
**Problema:** o hub tinha fundo 2D simples; Ricaliff pediu o nível gráfico do site Pulsar (engenharia do estudio-imersivo) sem arriscar um app funcional de 250KB. · **Opções:** A) reescrever visual dentro do app.js · B) módulo aditivo (fx.js) que engancha por fora. · **Decisão:** B — fx.js com 3 módulos: void GL (fragment shader theme-aware que lê --primary/--accent/--bg e recolore via MutationObserver no data-theme), intro 1×/sessão (símbolo geométrico se desenha e funde no app, sessionStorage), reveals com stagger (monkey-patch de activateView — funciona porque os scripts são clássicos e a resolução é pelo escopo global em runtime). void.js vira fallback (dataset.off aposenta o pintor 2D quando o GL assume; sem WebGL ele segue). · **Por quê:** cirurgia aditiva = risco zero na lógica; shader lê tokens = os DOIS temas (Gojo/pro) ganham o fundo de graça. · **Pegadinha honrada:** NUNCA gatear no prefers-reduced-motion (Windows/RDP liga sozinho — bug pago 2×); escape explícito é ?static=1. Disciplina de app: 30fps, DPR capado, pausa em aba oculta, teto de 16 filhos no stagger. · **Em entrevista (30s):** "Portei uma engine WebGL de landing imersiva pra dentro de um dashboard em produção sem tocar na lógica: um módulo aditivo que lê os design tokens do CSS como uniforms e intercepta a troca de view por monkey-patch — o app não sabe que a camada existe, e removê-la é apagar um script tag."

## 2026-07-22 — [trilha] Camada disciplina: syllabus + carga horária, não módulos inflados
**Problema:** a trilha inteira somava 14,5h de leitura — 165 módulos bons como aulas, mas sem o resto que forma alguém (uma graduação são ~3.000h). Feedback do Ricaliff: "módulos curtos demais pra sair formado". · **Opções:** A) inflar os 165 .md para 3-4× o tamanho · B) camada disciplina: módulos ficam como aulas; cada trilha ganha SYLLABUS.md com carga horária real decomposta, bibliografia obrigatória POR módulo (obra consagrada por capítulo nomeado + horas), 2-3 labs isolados do zero com spec e "pronto quando", e critério de formatura · C) híbrido. · **Decisão:** B (escolha do Ricaliff, com labs isolados por preferência dele). · **Por quê:** na faculdade a aula também é curta — as outras 58h da disciplina são bibliografia, listas e labs. Texto meu inflado seria parede de texto pior que Kleppmann/docs oficiais que já existem; a hora de formação mora na leitura dirigida + prática, não no resumo. · **Consequências:** 23 syllabi (921h de formação total no índice), horas visíveis na UI, SYLLABUS.md excluído dos 3 geradores (não é módulo nem card nem quest). Bibliografia citada por NOME de capítulo/seção — nunca número ou URL incerto (regra anti-alucinação). · **Em entrevista (30s):** "Meu hub media leitura; formação se mede em horas de prática dirigida. Em vez de inflar o texto, estruturei cada trilha como disciplina: syllabus com bibliografia obrigatória por capítulo, labs com critério de pronto e formatura só com projeto aceito em revisão. 921 horas mapeadas — escala de graduação, com evidência em commit no lugar de diploma."

## 2026-07-22 — [app] Scrum solo único + status window Solo Leveling
**Problema:** gestão fragmentada (Tarefas + Quest Board + focus do dia) diluía o foco; Ricaliff pediu um método só (Scrum), gamificação estilo Solo Leveling cobrindo treino físico, e o app centrado em formar. · **Opções:** A) Scrum ao lado dos sistemas atuais · B) Sprint semanal ENGOLE Tarefas/Quest Board (dados migram pro backlog 1×, nav enxuga) + hunter.js substitui o player card. · **Decisão:** B (escolha do Ricaliff). · **Por quê:** dois sistemas de gestão = zero método; a simplificação é o pedido. Kanban semanal com WIP 2 e done-com-evidência é Scrum reduzido ao que um dev solo executa de verdade. Ranks E→S calibrados na escala REAL do produtor (str~200 após meses de grind; S=300 aspiracional) — INT/CHA no E expõem o gap que a formação preenche. Daily quest com penalidade (streak zera) é a mecânica Solo Leveling que cria constância; treino físico entra na mesma economia de atributos (STR/CON/DEX), sem moeda nova. · **Consequências:** views antigas de Tarefas/Quest Board continuam no código (dados preservados) mas fora da nav — remoção definitiva é dívida registrada; pegadinha documentada: `let SYNC` no app.js é global LEXICAL (não `window.SYNC`) — módulo externo testa `typeof SYNC`. · **Em entrevista (30s):** "Consolidei três sistemas de gestão num Scrum solo com WIP e evidência obrigatória, e calibrei a gamificação com dados reais de uso em vez de números inventados — a curva de rank nasceu da distribuição efetiva de atributos, não de chute."

## 2026-07-28 — [guitarra] Diagramas na visão do jogador, não na notação de cifra
**Problema:** Ricaliff (zero música) não entendeu a página v1 — texto e diagramas padrão pressupõem vocabulário.
**Opções:** A) notação de cifra vertical (padrão da indústria) · B) visão do jogador (braço horizontal, corda grossa embaixo, âncoras "perto do seu rosto") · C) fotos.
**Decisão:** B, em TODOS os diagramas + demos interativas (vibração, encurtar corda) antes de qualquer acorde.
**Por quê:** cifra padrão é espelhada em relação ao que o iniciante vê no colo — exige tradução mental que ele ainda não tem. Consistência de ponto de vista > convenção. C exigiria assets externos (página é zero-dep).
**Consequências:** conteúdo musical futuro no hub deve manter a mesma orientação; se um dia ensinar leitura de cifra, precisa de uma seção "traduzindo pro formato padrão".
**Em entrevista (30s):** "Escolhi o ponto de vista do usuário sobre a convenção da indústria porque o público era iniciante absoluto: convenção serve a quem já internalizou o domínio; pra quem sabe zero, cada tradução mental é um ponto de desistência."
